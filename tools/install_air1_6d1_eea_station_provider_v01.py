...
ROOT = Path.cwd()
SNAPSHOT = ROOT / "dev-snapshots" / f"AIR1_6D1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def test():
    subprocess.run(
        [sys.executable, "-m", "pytest", "tests/test_eea_station_provider.py"],
        check=True
    )

def main():
    print("=== AIR-1.6D1 EEA Station Provider installer ===")
    verify()
    backup()
    install()
    test()
